...
- Frontend → Backend: Use `http://backend:8000` (internal DNS)
- Backend → Backend: Use `http://backend:8000` (same service)

**External (from browser):**
- Frontend → Backend: Use `process.env.NEXT_PUBLIC_API_URL` (public URL)
...
**Deployment:** ✅ App Platform (Automatic)
