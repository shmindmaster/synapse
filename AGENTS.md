...
- **TLS Certificate**: Wildcard `wildcard-shtrial-tls` in `ingress-nginx` namespace
- **Domains**: 
...
5. Syncs wildcard TLS certificate
...
6. **TLS and Ingress**:
   - Always use `wildcard-shtrial-tls` secret (synced from `ingress-nginx` namespace)
...
- Create per-app TLS certificates
- Use non-standard service names
- Modify shared cluster configuration
- Use port 25061 for writes (use 25060)
