...
web: cd src/api && npm install -g pnpm@10.23.0 && pnpm install && pnpm exec prisma generate && pnpm run build && node dist/server.js

# Release phase - runs before deployment

# Handles database migrations automatically

release: cd src/api && npm install -g pnpm@10.23.0 && pnpm install && npx prisma migrate deploy
