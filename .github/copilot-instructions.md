# 👑 PRINCIPAL ARCHITECT MANDATE: SHTrial Platform Standards (v8.6)
...
### Reference: `shtrial-demo-standards.md`

This platform is documented in `shtrial-demo-standards.md` which defines:
- Shared infrastructure topology (cluster, database, storage, networking)
...
**Your mission:** Apply these standards consistently across all applications while allowing flexibility for app-specific needs.
...
| **Container Registry** | `shtrial-reg` | DO Container Registry |
| **DNS Zone** | `shtrial.com` | Managed via DO Networking |
| **Load Balancer** | NGINX Ingress Controller | Shared LB for all apps |
| **Load Balancer IP** | `152.42.152.118` | Single public entry point |
...
| **STT/Whisper** | `http://whisper-service.ai-services.svc.cluster.local:9000/transcribe` | In-cluster CPU service |
...
- **Service Names:** `{APP_SLUG}-frontend`, `{APP_SLUG}-backend`
- **Ingress Hosts:**
...
- **Container Images:**
  - Backend: `registry.digitalocean.com/shtrial-reg/{APP_SLUG}-api:{TAG}`
  - Frontend: `registry.digitalocean.com/shtrial-reg/{APP_SLUG}-web:{TAG}`
...
## 📝 7. STANDARD `.env.shared` TEMPLATE

Every application repository should have an `.env.shared` file with these variables:
...
# Internal CPU Services (Cluster Local)
...
Standard deployment automation script that:
1. Loads environment from `.env.shared`
2. Builds and pushes Docker images
3. Generates Kubernetes manifests with `envsubst`
4. Applies manifests to cluster
5. Runs database migrations (if `scripts/migrate.sh` exists)
...
kubectl logs -n {APP_SLUG} -l app={APP_SLUG}-backend --tail=20
...
- **Never commit** `.env.shared` with real credentials to git
...
- **`shtrial-demo-standards.md`** - Complete platform architecture and standards
- **`.pendoah/platform/docs/01-platform-overview.md`** - Platform overview
- **`.pendoah/platform/docs/04-application-standards.md`** - Application standards
- **`.pendoah/platform/docs/99-coding-standards.md`** - Coding standards
- **`.pendoah/platform/templates/`** - Reference templates for Docker, K8s, and code
...
- [ ] `.env.shared` follows the standard template
...
- [ ] Kubernetes manifests use template variables (`${VAR}`)
...
**Version:** 8.6 (SHTrial Platform Standards)
