<<<<<<< H:/Repos/shmindmaster/synapse/src/web/src/App.tsx
...
function Dashboard() {
  ...
  const handleIndexFiles = useCallback(async () => {
    ...
    try {
      ...
      // Prepare file data for backend — content-aware chunking preserves
      // code structure, markdown sections, and prose paragraphs.
      // No arbitrary chunk cap: the chunker's internal limits (500 chunks,
      // 1MB text) handle safety. Each chunk becomes a separate vector.
      const fileData = files.map(file => {
        const MAX_CONTENT_LENGTH = 1024 * 1024; // 1MB
        const safeContent = file.content.length > MAX_CONTENT_LENGTH 
          ? file.content.slice(0, MAX_CONTENT_LENGTH) 
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          // Content-aware chunking (code/markdown/prose)
...
export default App;
>>>>>>> C:/Users/SaroshHussain/.windsurf/worktrees/synapse/synapse-0a1ab70e/src/web/src/App.tsx
