...
export class VectorStoreService {
        ...
        try {
            ...
            for (const doc of documents) {
                ...
                if (doc.embedding) {
                    ...
                    await client.query(`INSERT INTO vector_embeddings (id, path, content, embedding, metadata, preview, indexed_at, updated_at)
             ...
             ON CONFLICT (path) DO UPDATE SET
               ...
               updated_at = NOW()`, [doc.path, doc.content, embeddingArray, metadataJson, doc.preview || null]);
                }
                else {
             ...
             ON CONFLICT (path) DO UPDATE SET
               ...
               metadata = EXCLUDED.metadata,
               preview = EXCLUDED.preview,
               updated_at = NOW()`, [doc.path, doc.content, metadataJson, doc.preview || null]);
                }
            }
    ...
    async textSearch(query, limit = 10, filters) {
        ...
        ${whereClause}
        ORDER BY score DESC
        LIMIT $${paramIndex + queryWords.length}`, [...params, ...queryWords.map(w => `%${w}%`), limit]);
            return result.rows.map((row) => ({
                id: row.id,
                path: row.path,
                content: row.content,
